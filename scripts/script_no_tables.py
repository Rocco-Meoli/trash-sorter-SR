...
# --- Slot fissi sul ground (random spawn) ---
SLOT_X = 0.60
SLOT_YS = [-0.18, 0.0, 0.18]

# PRIMA era 0.42 (su tavolo alto 0.40). Ora su ground:
SLOT_Z = 0.02
...
class OverheadVision:
    ...
    def build_tasks_from_image(self):
        ...
        colors_sorted = sorted(det.keys(), key=lambda k: det[k][0])  # left->right
        if FLIP_LEFT_RIGHT:
...
# ===============================
def main():
    rospy.init_node("trash_sort_camera_random_no_tables")
